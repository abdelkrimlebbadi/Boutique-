-- Extends admin_save_product (20260728130100) so each variant's print area
-- (product_variants.print_area_width_px/height_px/dpi, added in
-- 20260728140000) is saved from the admin ProductForm. p_variants is
-- already arbitrary jsonb per element, so the function signature is
-- unchanged — CREATE OR REPLACE is sufficient, same posture as the other
-- customization-feature RPC updates.
  ...
  for variant in select * from jsonb_array_elements(p_variants) loop
    if variant ->> 'id' is not null then
      update product_variants set
        ...
        print_area_width_px = nullif(variant ->> 'print_area_width_px', '')::integer,
        print_area_height_px = nullif(variant ->> 'print_area_height_px', '')::integer,
        print_area_dpi = nullif(variant ->> 'print_area_dpi', '')::integer
      where id = (variant ->> 'id')::uuid
      returning id into v_variant_id;
    else
      insert into product_variants (
        product_id, sku, size, color, printful_variant_id, stock_policy, weight_grams,
        print_area_width_px, print_area_height_px, print_area_dpi
      )
      values (
        v_product.id,
        variant ->> 'sku',
        ...
        nullif(variant ->> 'print_area_width_px', '')::integer,
        nullif(variant ->> 'print_area_height_px', '')::integer,
        nullif(variant ->> 'print_area_dpi', '')::integer
      )
      returning id into v_variant_id;
    end if;
